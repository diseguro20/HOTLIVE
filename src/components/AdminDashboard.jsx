            ...
            <div className="admin-detail-overlay" role="presentation" onMouseDown={() => setSelectedOrder(null)}>
              <section className="admin-detail-dialog" role="dialog" aria-modal="true" aria-label="Detalhes do pedido" onMouseDown={(event) => event.stopPropagation()}>
                ...
                <div className="admin-detail-grid">
                  ...
                  <div><span>Contato protegido</span><strong>Telefone final {selectedOrder.customerPhoneLast4 || '-'}</strong><small>Documento final {selectedOrder.customerDocumentLast4 || '-'}</small></div>
                  <div><span>Pagamento</span><strong>{selectedOrder.method === 'credit_card' ? 'Cartão de crédito' : 'PIX'}</strong><small>{selectedOrder.cardBrand || 'Sem bandeira'} {selectedOrder.cardLast4 ? `•••• ${selectedOrder.cardLast4}` : ''}</small></div>
                  <div><span>Valor e pacote</span><strong>{money(selectedOrder.amount)}</strong><small>{number(selectedOrder.coins)} moedas</small></div>
                  <div className="admin-detail-address"><span>Endereço de cobrança</span><strong>{selectedOrder.billingAddress?.street || '-'}, {selectedOrder.billingAddress?.number || '-'}</strong><small>{selectedOrder.billingAddress?.district || '-'} · {selectedOrder.billingAddress?.city || '-'} / {selectedOrder.billingAddress?.state || '-'} · CEP {selectedOrder.billingAddress?.postalCode || '-'}</small></div>
                  <div className="admin-detail-address"><span>Antifraude</span><strong>Aguardando integração do motor de risco</strong><small>Score, decisão e sinais serão associados ao pedido sem armazenar dados completos do cartão.</small></div>
                </div>
              </section>
            </div>
          )}
